...
#include <math.h>
	...
	public:
		...
		inline Vector2D   makeRotated ( float radians ) const;
	...
	Vector2D Vector2D::unit () const {
		float abs = length();
		if ( abs <= VECTOR2D_EPSILON ) // Prevent division by zero
			return (*this);
		else
			return Vector2D(
				x / abs,
	...
	Vector2D Vector2D::project ( const Vector2D & onto ) const {
		...
		if ( fabs(onto * onto) < VECTOR2D_EPSILON ) 
			return Vector2D( 0, 0 );
	...
	Vector2D Vector2D::makeRotated ( float radians ) const {
	...
	void Vector2D::rotate ( float radians ) {
		...
		float newX = x * cosMR + y * sinMR;
		float newY = y * cosR  - x * sinMR;
